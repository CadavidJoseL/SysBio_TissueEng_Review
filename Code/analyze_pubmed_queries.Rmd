...
pre-checked files. PMC retrieves too many non-specific hits, so exclude

```{r}
# Do in chunks of 50 (although internally stuff is saved every 10 prompts)
IDs <- tables[["PMID_extra"]]$PMID %>% unique()
IDs <- setdiff(IDs, 
               c(tables_mixed[["all_gen_PMC"]]$PMID, 
                 tables_mixed[["all_gen"]]$PMID, 
                 tables_mixed[["TE_SB_gen"]]$PMID))

extra_all_gen_unique <- verify_titles(IDs,
...
Producing a list of plots. These wrapper functions save the plots
